...
- **TTS relicensing (shipping gate)** — Kokoro's natural English voice uses **espeak-ng g2p, which is GPL-3.0**; the prototype ships with it (best quality, zero friction), so the distributed binary would inherit GPL-3.0. If a permissive license is required for distribution, swap to a non-GPL g2p (Misaki / dictionary-based) before ship. Decide when packaging (0006) firms up.
